...
export const questionRouter = router({
  ...
  create: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // 選択肢を作成（SA/MA の場合）
      if (
        (questionData.type === "SA" || questionData.type === "MA") &&
        options &&
        options.length > 0
      ) {
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (const section of sections) {
        ...
        let questions: Array<{
          ...
          {
            type: "SA",
            title: `${section.title}に関する設問（単一選択）`,
            options: buildOptions(["はい", "いいえ", "わからない"]),
          },
          {
            type: "MA",
            title: `${section.title}に関する設問（複数選択）`,
            options: buildOptions(["価格", "品質", "使いやすさ", "サポート"]),
          },
          {
            type: "FA",
            title: `${section.title}に関する自由記述`,
        ...
        if (isScreening) {
          if (title.includes("基本属性")) {
            questions = [
              {
                type: "SA",
                title: "性別を教えてください",
                options: buildOptions(["男性", "女性", "回答しない"]),
              },
              {
                type: "SA",
                title: "年齢を教えてください",
            ...
            questions = [
              ...
              {
                type: "FA",
                title: "対象者条件に関する補足があれば教えてください",
                placeholder: "例：カテゴリの具体例 など",
              },
            ];
            ...
            questions = [
              {
                type: "SA",
                title: "婚姻状況を教えてください",
                options: buildOptions(["未婚", "既婚", "離別/死別"]),
              },
              {
                type: "MA",
                title: "同居家族を教えてください（複数選択可）",
                options: buildOptions([
                  "配偶者",
                  "子ども",
                  "親",
          ...
          if (title.includes("利用状況") || title.includes("体験")) {
            ...
            questions = [
              ...
              {
                type: "FA",
                title: "最近の購入行動で気づいた点があれば教えてください",
                placeholder: "自由記述",
              },
            ];
          } else if (
        ...
        for (let i = 0; i < questions.length; i++) {
          ...
          const created = await ctx.db.question.create({
            data: {
              ...
              order: i + 1,
              config:
                qDef.type === "FA" && qDef.placeholder
                  ? JSON.stringify({ placeholder: qDef.placeholder, rows: 3 })
                  : "{}",
            },
          });
