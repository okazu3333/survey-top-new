...
export default function TestSpecViewerPage() {
	...
	const spec = useMemo(() => (items && items.length > 0 ? items[0] : null), [items]);
	...
	useEffect(() => {
		...
		const computedScCount = Array.isArray(sc?.questions) ? sc.questions.length : 0;
		const computedMainCount = Array.isArray(main?.questions) ? main.questions.length : 0;
		...
		const payload = {
			...
			targetCondition: Array.isArray(basic.targetConditions) ? basic.targetConditions.join(" / ") : "",
			analysisCondition: Array.isArray(basic.analysisTargets) ? basic.analysisTargets.join(" / ") : "",
		...
		<div className="p-4 space-y-4">
			<div className="flex items-center gap-3">
				...
				<Select value={caseType} onValueChange={setCaseType}>
					<SelectTrigger className="w-[160px]"><SelectValue placeholder="Case" /></SelectTrigger>
					<SelectContent>
						<SelectItem value="normal">normal</SelectItem>
						<SelectItem value="abnormal">abnormal</SelectItem>
					</SelectContent>
				</Select>
				<Button variant="outline" onClick={fetchData} disabled={loading}>再取得</Button>
				{spec ? (
					<div className="flex items-center gap-2 text-sm text-muted-foreground">
						<Badge variant="secondary">{spec.specId}</Badge>
						{spec.isActive ? <Badge>active</Badge> : <Badge variant="outline">inactive</Badge>}
					</div>
				) : null}
			</div>
			...
			<div className="grid grid-cols-12 gap-4">
				<Card className="col-span-4 p-3 space-y-2">
					...
					<Textarea value={conditions} onChange={(e) => setConditions(e.target.value)} className="min-h-[260px] font-mono text-xs" />
					<div className="text-xs text-muted-foreground">調査タイトル／目的／対象者条件／分析対象者条件／SC/本調査の設問数</div>
				</Card>
				...
				<Card className="col-span-8 p-3">
					...
					{loading ? (
						...
						<div className="text-sm text-muted-foreground">該当データがありません</div>
					) : (
						<ScrollArea className="h-[520px] pr-3">
							<div className="space-y-4">
								<div className="text-sm text-muted-foreground">{surveySpec?.surveyTitle || spec.title}</div>
											...
											{(sec.questions || []).map((q: any, qi: number) => (
												<div key={qi} className="text-sm">
													<div className="flex items-center gap-2">
														...
														<div className="font-medium">[{q.code}] {q.title}</div>
													</div>
													{Array.isArray(q.options) && q.options.length > 0 && (
														<ul className="list-disc pl-6 mt-1 text-muted-foreground">
															{q.options.map((o: any, oi: number) => (
																<li key={oi}>[{o.code}] {o.label}</li>
															))}
														</ul>
													)}
												</div>
			...
			<Card className="p-3">
				...
				<pre className="whitespace-pre-wrap break-words text-xs bg-muted/30 p-2 rounded">{JSON.stringify(spec?.expectations || {}, null, 2)}</pre>
			</Card>
		</div>
	);
} 