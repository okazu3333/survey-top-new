...
const Page = ({ params }: Props) => {
  ...
  useEffect(() => {
    console.log("Screening questions state:", screeningQuestions);
  }, [screeningQuestions]);

  useEffect(() => {
    console.log("Main survey questions state:", mainSurveyQuestions);
  }, [mainSurveyQuestions]);
  ...
  const { data: sections, isLoading, error: sectionsError } = api.section.list.useQuery({
    surveyId,
  });

  // Debug sections data
  useEffect(() => {
    console.log("Sections API data:", { sections, isLoading, sectionsError, surveyId });
  }, [sections, isLoading, sectionsError, surveyId]);

  // Re-enabled for BigQuery
  const { data: survey, isLoading: surveyLoading, error: surveyError } = api.survey.getById.useQuery({
    id: surveyId,
  });

  // Debug survey data
  useEffect(() => {
    console.log("Survey data:", { survey, surveyLoading, surveyError, surveyId });
  }, [survey, surveyLoading, surveyError, surveyId]);

  // Removed heavy questionsData query - not needed on sections page
  ...
  // Read-only mode: Section sync is disabled
  const autoSyncSections = {
    mutate: () => {},
    mutateAsync: async () => ({ success: false, created: 0 }),
    isPending: false,
  };
  ...
  useEffect(() => {
    ...
    if (sections && Array.isArray(sections)) {
      ...
      const screeningSections = sections
        .filter((s: any) => {
          console.log("Section phase:", s.phase, "Type:", typeof s.phase);
          return s.phase === "SCREENING";
        })
        .map((s: any) => s.title);
      ...
      console.log("Screening sections:", screeningSections);
  ...
  useEffect(() => {
    const run = async () => {
      console.log("Auto-generation check:", { 
        ...
        isPending: autoSyncSections.isPending 
      });
      ...
      if (
        ...
        console.log("Generating sections for survey:", survey);
        const autoGeneratedScreening = generateScreeningSections(survey);
        const autoGeneratedMain = generateMainSections(survey);
        
        console.log("Generated screening:", autoGeneratedScreening);
        console.log("Generated main:", autoGeneratedMain);
